...
API.interceptors.response.use(
  (response) => response,
  (error) => {
    const authEndpoints = ['/login', '/register', '/verify-email', '/forgot-password', '/reset-password', '/invitations/accept'];
...
export const sendChatMessage = async (
  ...
  try {
    ...
    const axiosError = error as { response?: { status?: number; data?: ChatLimitErrorResponse } };
    if (axiosError.response?.status === 429 && axiosError.response?.data?.usage) {
      return {
        ok: false,
        status: 429,
