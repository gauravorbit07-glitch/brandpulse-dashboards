...
export const ProgressAcrossRuns = () => {
  ...
  const visibleRuns = showAll ? trendRuns : trendRuns.slice(0, MAX_ROWS);
  const hasMore = trendRuns.length > MAX_ROWS;
    ...
    <div className="bg-card rounded-xl border border-border p-4 md:p-6 hover:shadow-md transition-shadow">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-2">
          <h4 className="text-sm font-semibold text-foreground">All Runs</h4>
          <span className="text-[10px] text-muted-foreground">
            ▲▼ change vs the previous run, shown only when the same keyword set has been run before
          </span>
        </div>
        ...
        {hasMore && (
          ...
          >
            {showAll ? `Show less ↑` : `Show ${trendRuns.length - MAX_ROWS} more runs ↓`}
          </button>
        )}
      </div>
    </div>
