  ...
  const insight = useMemo(() => {
    ...
    if (selectedKeyword === 'all') {
      ...
      if (brandRank === 1) {
        ...
        return `Not mentioned for "${selectedKeyword}" — opportunity to improve`;
      } else {
    ...
    <div className="bg-card rounded-2xl border border-border/60 p-5 md:p-6 overflow-hidden shadow-card hover:shadow-elevated transition-shadow duration-300">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-foreground">Mention Distribution</h3>
        </div>
        <div className="relative">
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="flex items-center gap-2 px-3 py-1.5 bg-muted/60 rounded-lg text-xs font-medium text-foreground hover:bg-muted transition-colors"
          >
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 top-full mt-1 w-56 bg-card border border-border rounded-xl shadow-elevated z-50 max-h-60 overflow-y-auto">
              <button
      ...
      {/* Dynamic insight */}
      <p className="text-xs text-muted-foreground text-center mt-3 pt-3 border-t border-border/50 italic">
        💡 {insight}
      </p>
    </div>
  );
