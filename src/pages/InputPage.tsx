...
export default function InputPage() {
          ...
          if (Array.isArray(analytics.brands)) {
            ...
            setSelectedCompetitors((prev) => {
              ...
              competitors.forEach((nc: OnboardingCompetitor) => {
                if (!merged.some((pc) => isSameCompetitor(nc, pc)) && merged.length < 5) {
                  merged.push(nc);
                }
              });
              return merged;
  ...
  const handleGenerateBrandDescription = async (
    ...
    try {
      ...
      if (!brandName && fallbackBrand) {
        setBrandName(fallbackBrand);
      }

      const suggestions = await getOnboardingSuggestions({
        website: normalizeDomain(url),
        name: brandName || fallbackBrand || undefined,
        business_domain: "General",
      });

      if (suggestions) {
        if (!brandName && suggestions.product_name) {
          setBrandName(suggestions.product_name);
        }
        ...
        setSuggestedCompetitors((prev) => {
          const merged = [...prev];
          incomingCompetitors.forEach((incoming) => {
            if (!merged.some((existing) => isSameCompetitor(existing, incoming))) {
      ...
      if (productIdParam || productId) {
        ...
        if (pid) {
          const res = await getProductAnalytics(pid, accessToken);
          const analytics = res?.analytics?.[0]?.analytics;

          if (analytics?.executive_summary?.conclusion) {
            setBrandDescription(analytics.executive_summary.conclusion);
          }

          if (analytics?.brand_name && !brandName) {
            setBrandName(analytics.brand_name);
          }

          if (analytics?.search_keywords) {
            const nextKeywords = Object.values(analytics.search_keywords)
              .map((kw: any) => kw?.name)
              .filter((kw: any) => typeof kw === "string" && kw.trim().length > 0);

            setSuggestedKeywords((prev) => {
              const uniqueNew = nextKeywords.filter((k: string) => !prev.includes(k));
                    ...
                    <div className="flex flex-col gap-8 relative">
                      ...
                      {[
                        ...
                        },
                        {
                          ...
                          completed: selectedCompetitors.length >= 4,
                        },
                        {
                          id: 3,
                          label: "Keywords",
                          desc: "Visibility Targets",
                          active: selectedCompetitors.length >= 4,
                          ...
                          {selectedCompetitors.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedCompetitors.map((competitor, idx) => (
                                <Badge
                                  key={`${normalizeDomain(competitor.website || competitor.name)}-${idx}`}
                                  variant="secondary"
                      ...
                      <CardContent className="space-y-8 p-8 pt-0">
                        <div className="flex flex-wrap gap-2 min-h-[40px] p-4 rounded-lg border bg-muted/30">
                          {selectedKeywords.length === 0 ? (
                            <p className="text-xs text-muted-foreground/50 w-full text-center">
                              Add Keywords
                            </p>
                          ) : (
            ...
            <div className="min-h-[85vh] flex flex-col items-center justify-center p-8 relative overflow-hidden">
              ...
              <div className="w-full max-w-4xl grid lg:grid-cols-2 gap-16 items-center relative z-10">
                ...
                <div className="space-y-8 order-1 lg:order-2">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground/60">
                        Swarm Intelligence
                      </h3>
